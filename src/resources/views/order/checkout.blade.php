...
<div class="min-h-screen bg-[#f8fafc] py-12 pt-32">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @if(session('error'))
            <div class="mb-6 p-4 rounded-xl border border-red-200 bg-red-50 text-red-700 font-bold max-w-3xl mx-auto">{{ session('error') }}</div>
        @endif
        @if ($errors->any())
            <div class="mb-6 p-4 bg-red-50 text-red-700 text-sm rounded-xl font-bold max-w-3xl mx-auto">Terdapat kesalahan pada inputan Anda. Silakan periksa kembali email dan password.</div>
        @endif

        {{-- ROUTING KOMPONEN VIEW BERDASARKAN STATE --}}
        @if(!session('order.payment_method'))
            @include('order.checkout.check1_payment')
            
        @elseif(!Auth::check())
            @include('order.checkout.check2_auth')
            
        @else
            @include('order.checkout.check3_confirm')
        @endif
