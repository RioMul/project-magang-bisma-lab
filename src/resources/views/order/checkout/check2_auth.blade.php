<div class="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start" x-data="{ tab: '{{ (old('name') || $errors->has('name') || $errors->has('password') || ($errors->has('email') && old('name'))) ? 'register' : 'login' }}' }">
    ...
    <div class="lg:col-span-7 space-y-6">
        <div class="mb-8 flex items-center justify-between">
            <div class="flex items-center gap-4">
                <div class="w-10 h-10 rounded-full bg-[#057A55] text-white flex items-center justify-center font-bold">✓</div>
                <div>
                    <h1 class="text-3xl font-black text-slate-900 tracking-tight">Buat Akun Dulu!</h1>
            ...
            <form action="{{ route('order.checkout.reset_payment') }}" method="POST">
                @csrf
                <button type="submit" class="text-xs font-bold text-slate-400 hover:text-[#0369a1] underline">← Ganti Bank</button>
            </form>
        </div>

        <div class="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
            <div class="flex rounded-xl bg-slate-100 p-1.5 mb-8">
                <button type="button" @click="tab = 'register'" :class="tab === 'register' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-900'" class="flex-1 py-3 text-xs font-bold uppercase tracking-wider rounded-lg transition-all">Buat Akun Baru</button>
                <button type="button" @click="tab = 'login'" :class="tab === 'login' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-900'" class="flex-1 py-3 text-xs font-bold uppercase tracking-wider rounded-lg transition-all">Sudah Punya Akun</button>
            </div>
    ...
    <div class="lg:col-span-5 sticky top-28 space-y-6">
        ...
        <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
            ...
            <div class="relative h-64 sm:h-72 bg-slate-900 border-b border-slate-100 group overflow-hidden shrink-0">
                ...
                <div class="absolute inset-0 bg-gradient-to-t from-slate-900/90 via-slate-900/30 to-transparent flex items-end p-6 sm:p-8">
                    <div class="w-full">
                        ...
                        <h3 class="text-3xl font-black text-white drop-shadow-md">{{ $template->name }}</h3>
                        <p class="text-sm text-slate-200 mt-2 line-clamp-2 drop-shadow-md">{{ $template->description }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6 sm:p-8 flex-1 flex flex-col">
                <div class="bg-slate-50 rounded-2xl p-5 border border-slate-100 space-y-5 mb-6">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="font-bold text-slate-900 text-sm">{{ $domain }}</p>
                        </div>
                    </div>
                ...
                <div class="space-y-3 text-sm px-2 mb-6 flex-1">
                    <div class="flex justify-between items-center"><span class="font-bold text-slate-500">Biaya Paket</span><span class="font-bold text-slate-800">Rp {{ number_format($package->price_annually, 0, ',', '.') }}</span></div>
                    <div class="flex justify-between items-center"><span class="font-bold text-slate-500">Registrasi Domain</span><span class="font-bold text-slate-800">Rp {{ number_format($domainPrice, 0, ',', '.') }}</span></div>
                </div>

                <div class="pt-5 border-t border-slate-100 flex justify-between items-end px-2">
                    <span class="font-black text-slate-800 uppercase tracking-wider text-xs mb-1">Total Dibayar</span>
                    <div class="text-3xl font-black text-[#0369a1]">Rp {{ number_format($totalAmount, 0, ',', '.') }}</div>
                </div>
                
                <p class="mt-6 text-xs text-center text-slate-400 font-bold bg-slate-50 p-4 rounded-xl border border-slate-100">Selesaikan registrasi di sebelah kiri...</p>
            </div>
        </div>
    </div>
</div>