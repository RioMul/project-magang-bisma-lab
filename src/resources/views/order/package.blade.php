            ...
            <div class="lg:col-span-5 space-y-6 sticky top-28">
                ...
                @if($selectedTemplate)
                    <div class="bg-white rounded-3xl border border-slate-200 p-4 shadow-sm">
                        <div class="bg-slate-900 rounded-2xl overflow-hidden mb-4 relative aspect-[4/3]">
                            <img src="{{ asset($selectedTemplate->images->where('is_primary', true)->first()->image_path ?? 'tech1.png') }}" class="w-full h-full object-cover">
                        </div>
                        <div class="px-2 pb-2">
                            <span class="text-[10px] text-[#0369a1] font-black uppercase tracking-wider">Selected Template</span>
                            <div class="flex items-center justify-between mt-1">
                                <h3 class="font-bold text-slate-900 text-lg">{{ $selectedTemplate->name }}</h3>
                                <a href="{{ route('order.template') }}" class="text-xs font-bold text-[#0369a1] hover:underline flex items-center gap-1">Change <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg></a>
                            </div>
                        </div>
                    </div>
                @endif
                ...
                @if($selectedDomain)
                    <div class="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm flex items-center justify-between">
                        <div>
                            <span class="text-[10px] text-[#057A55] font-black uppercase tracking-wider">Selected Domain</span>
                            <h3 class="font-bold text-slate-900 text-lg mt-1">{{ $selectedDomain }}</h3>
                        </div>
                        <a href="{{ route('order.domain') }}" class="w-10 h-10 rounded-full bg-slate-50 text-slate-400 hover:bg-sky-50 hover:text-[#0369a1] flex items-center justify-center transition">
                    ...
                    <div x-data="{ expanded: {{ ($selectedPackageId == $pkg->id || (!$selectedPackageId && $pkg->is_popular)) ? 'true' : 'false' }} }" 
                        ...
                        @if($pkg->is_popular)
                            <div class="bg-sky-50 px-6 py-2 border-b border-sky-100 flex items-center gap-2">
                                <svg class="w-4 h-4 text-[#0369a1]" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"></path></svg>
                                <span class="text-[10px] font-black text-[#0369a1] uppercase tracking-wider">Most Popular</span>
                            </div>
                        @endif

                        <div class="p-6 sm:p-8 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                            <div class="flex-1">
                                <h3 class="font-black text-slate-900 text-xl">{{ $pkg->name }}</h3>
                                <p class="text-xs text-slate-500 mt-1 mb-4">{{ $pkg->short_description }}</p>
                                <div class="text-3xl font-black text-slate-900 flex items-end gap-1">
                                    Rp {{ number_format($pkg->price_monthly, 0, ',', '.') }}<span class="text-xs font-bold text-slate-400 mb-1.5">/ thn</span>
                                </div>
                            </div>
                            
                            <div class="flex flex-col items-end gap-3 shrink-0 w-full sm:w-auto">
                                <form action="{{ route('order.package.store') }}" method="POST" class="w-full sm:w-auto">
                                    @csrf
                                    <input type="hidden" name="package_id" value="{{ $pkg->id }}">
                                    {{-- TOMBOL BIRU SOLID --}}
                                    <button type="submit" class="w-full sm:w-auto px-8 py-3 rounded-xl font-bold text-sm transition bg-[#0369a1] hover:bg-[#027ea8] text-white shadow-sm">
                                        Select {{ explode(' ', $pkg->name)[1] ?? $pkg->name }}
                                    </button>
                                </form>
                                <button type="button" @click="expanded = !expanded" class="text-[11px] font-bold text-slate-500 hover:text-slate-800 flex items-center gap-1 transition">
                                    View Features <svg class="w-3 h-3 transition-transform duration-300" :class="expanded ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M19 9l-7 7-7-7"></path></svg>
                                </button>
                            </div>
                        </div>
                                    ...
                                    @foreach($pkg->features as $feature)
                                        <li class="flex items-center text-sm">
                                            @if($feature->pivot->is_included)
                                                <svg class="w-4 h-4 text-[#057A55] mr-2 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>
                                                <span class="text-slate-700 font-medium">{{ $feature->name }}</span>
                                            @else
                                                <svg class="w-4 h-4 text-red-400 mr-2 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>
                                                <span class="text-slate-400">{{ $feature->name }}</span>
                                            @endif
                                        </li>
                                    @endforeach
                                </ul>
