<nav id="smartNavbar" class="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm transition-transform duration-300 ease-in-out">
    <div class="w-full max-w-[1400px] mx-auto px-4 sm:px-8 py-3 sm:py-4 flex items-center justify-between">
        ...
        <div class="hidden lg:flex items-center gap-8 xl:gap-10 text-base font-semibold text-gray-700">
            ...
            <a href="{{ route('order.template') }}"
               class="hover:text-[#0396c7] transition {{ request()->routeIs('order.template') ? 'text-[#0396c7] font-bold' : '' }}">
                Template
            </a>
            <a href="{{ route('order.package') }}"
               class="hover:text-[#0396c7] transition {{ request()->routeIs('order.package') ? 'text-[#0396c7] font-bold' : '' }}">
                Harga
            </a>
            <a href="{{ route('order.domain') }}"
               class="hover:text-[#0396c7] transition {{ request()->routeIs('order.domain') ? 'text-[#0396c7] font-bold' : '' }}">
                Domain
            </a>
        </div>
        ...
        <div class="flex items-center gap-2 sm:gap-3 shrink-0">
            @auth
                <div class="relative hidden sm:block" x-data="{ dropdownOpen: false }">
                    <button @click="dropdownOpen = !dropdownOpen"
                            class="flex items-center gap-3 text-sm font-semibold text-gray-700 bg-gray-50 hover:bg-gray-100 border border-gray-200 px-3 py-1.5 rounded-full transition">
                    ...
                    <div x-show="dropdownOpen"
                         ...
                         class="absolute right-0 mt-2 w-52 bg-white rounded-xl shadow-xl py-2 border border-gray-100"
                         style="display:none;">
                        ...
                        <a href="{{ route('dashboard') }}"
                           class="block px-4 py-2 text-sm text-gray-700 hover:bg-cyan-50 hover:text-[#0396c7] transition">
                            Dashboard
                        </a>
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit"
                                    class="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 transition font-medium">
                                Keluar
                            </button>
                        </form>
                    </div>
            ...
            @else
                <div class="hidden sm:flex items-center gap-2">
                    <a href="{{ route('login') }}"
                       class="text-sm sm:text-base font-bold text-gray-700 hover:text-[#0396c7] px-3 py-2 transition">
                        Log In
                    </a>

                    <a href="{{ route('order.template') }}"
                       class="text-sm sm:text-base font-bold text-white bg-[#0396c7] hover:bg-[#027ea8] px-4 sm:px-5 py-2.5 rounded-xl shadow transition">
                        Buat Website!
                    </a>
                </div>
            @endauth
            ...
            <button id="mobileMenuButton"
                    ...
                    aria-expanded="false"
                    class="lg:hidden w-11 h-11 flex items-center justify-center rounded-xl border border-gray-200 text-gray-700 hover:bg-gray-100 transition">
    ...
    {{-- MENU MOBILE --}}
    <div id="mobileMenu"
         class="lg:hidden max-h-0 overflow-hidden opacity-0 border-t border-transparent transition-all duration-300 ease-in-out">
        <div class="px-4 sm:px-8 py-4 space-y-1">
            <a href="{{ route('home') }}#solusi"
               class="mobile-link block px-4 py-3 rounded-xl font-semibold text-gray-700 hover:bg-cyan-50 hover:text-[#0396c7] transition">
                Solusi
            </a>
            ...
            @auth
                <div class="px-4 py-3">
                    ...
                    <a href="{{ route('dashboard') }}"
                       class="block px-4 py-3 mb-1 rounded-xl font-semibold text-gray-700 hover:bg-cyan-50 hover:text-[#0396c7] transition">
                        Dashboard
                    </a>
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit"
                                class="w-full text-left px-4 py-3 rounded-xl font-semibold text-red-600 hover:bg-red-50 transition">
                            Keluar
                        </button>
                    </form>
                </div>
            @else
                <div class="px-4 pb-4 grid grid-cols-2 gap-3">
                    <a href="{{ route('login') }}"
                       class="text-center py-3 rounded-xl border border-gray-300 font-bold text-gray-700 hover:bg-gray-100 transition">
                        Log In
                    </a>

                    <a href="{{ route('order.template') }}"
                       class="text-center py-3 rounded-xl bg-[#0396c7] hover:bg-[#027ea8] text-white font-bold transition">
                        Buat Website
                    </a>
                </div>
            @endauth
