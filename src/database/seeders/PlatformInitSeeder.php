...
class PlatformInitSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $domainExtension = DB::table('domain_extensions')
            ->where('extension', '.com')
            ->first();

        if (!$domainExtension) {
            $extensionId = DB::table('domain_extensions')->insertGetId([
                'name' => 'Dot Com',
                'extension' => '.com',
                'created_at' => now(),
                'updated_at' => now(),
            ...
            $extensionId = $domainExtension->id;
        }

        $domainPrice = DB::table('domain_prices')
            ->where('domain_extension_id', $extensionId)
            ->first();

        if (!$domainPrice) {
            DB::table('domain_prices')->insert([
                'domain_extension_id' => $extensionId,
                'price' => 150000,
                'billing_period' => 1,
                'created_at' => now(),
                'updated_at' => now(),
        ...
        $features = [
            'Landing' => Feature::firstOrCreate([
                'name' => 'Single Landing Page',
            ]),

            'Domain' => Feature::firstOrCreate([
                'name' => 'Domain .com',
            ]),

            'Multi' => Feature::firstOrCreate([
                'name' => 'Multi Page Website',
            ]),

            'Seo' => Feature::firstOrCreate([
                'name' => 'SEO Friendly',
            ]),

            'BasicEcom' => Feature::firstOrCreate([
                'name' => 'Basic E-Commerce',
            ]),

            'Ecom' => Feature::firstOrCreate([
                'name' => 'Advanced E-Commerce',
            ]),

            'Payment' => Feature::firstOrCreate([
                'name' => 'Payment Integration',
            ]),

            'Priority' => Feature::firstOrCreate([
                'name' => 'Priority Support',
            ]),

            'Custom' => Feature::firstOrCreate([
                'name' => 'Custom Features',
            ]),

            'Dedicated' => Feature::firstOrCreate([
